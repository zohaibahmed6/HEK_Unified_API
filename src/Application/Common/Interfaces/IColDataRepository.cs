...
public interface IColDataRepository
{
    ...
    Task<DataTable> GetCurrentPatientDataAsync(string practiceSuffix, string? patientId, CancellationToken ct = default);

    /// <summary>`PHCO.GetProviderData` - `[OnlineClaim].[uspGetProvider]`.</summary>
    Task<DataTable> GetProviderDataAsync(string practiceSuffix, string? patientId, CancellationToken ct = default);
    ...
    Task<DataTable> GetSessionDataAsync(string practiceSuffix, string? patientId, CancellationToken ct = default);
    ...
    Task<DataTable> GetDiagnosisDataAsync(string practiceSuffix, string? patientId, string? sortOrder, DateTime minDate, DateTime maxDate, CancellationToken ct = default);
    ...
    Task<DataTable> GetSurgeryDataAsync(string practiceSuffix, string? patientId, string? locationId, string? encounterId, CancellationToken ct = default);
    ...
    Task<long> SaveInvoiceAsync(string practiceSuffix, string? patientId, string? accountHolderId, string? encounterId, string? serviceName, string? serviceCode,
