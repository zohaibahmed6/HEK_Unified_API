...
- ~~KARO/ERMS Authenticate: tenant-registry routing didn't replicate the real legacy encounterId-suffix quirk~~ — **Fixed 2026-07-28**: real legacy source (`APIController.cs:41-58` for KARO, identical pattern in `ermsapi`'s `APIController.cs:71-74` for ERMS; both feed into `HSSDA.cs:813`'s `ConnIndiciDB{practiceid}` connection-string lookup) overwrites the practice context entirely when a 4th `encounterId` segment (environment) is present — practiceId/practiceCode get discarded, routing collapses to environment alone. The 2026-07-24 tenant-registry rework (`KaroRoutingResolver`/`ErmsRoutingResolver`) had NOT replicated this — it matched all 3 segments independently, causing `2147488418__901__FZZ999-B__local`-shaped encounterIds (the real, captured legacy format) to fail with "practice not registered" even with correct credentials. Fixed: both resolvers now apply the same overwrite quirk (4th segment present → route by Environment alone, ignoring PracticeId/PracticeCode). `TenantRegistryService.ResolveRouteAsync` rewritten to a single-tier match with no fallback cascade (Environment-only tier, PracticeId+PracticeCode tier, or PracticeId-alone tier — whichever the caller's encounterId shape implies) instead of one flat 3-column AND-match. `PracticeRegistryEntry.PracticeId/PracticeCode/Environment` made nullable (migration `MakePracticeRoutingColumnsNullable`) so each registry row only fills the column(s) its tier actually uses, rather than padding unused columns with the old `"-"` sentinel string. Verified live: all 3 real encounterId shapes (`901`, `901__FZZ999-B`, `901__FZZ999-B__local`) now route correctly per their real legacy precedence, and an unregistered environment correctly fails outright with no silent fallback. Full test suite still 66/66 pass. COL needed no separate fix - it already reuses `IErmsRoutingResolver` directly (COL shares ERMS's real `HSSDA`/session-table in legacy too), so this fix covers COL automatically.
- ~~Tier-3 (environment-only) registry rows weren't collision-safe~~ — **Fixed 2026-07-28**: `TenantRegistryService`'s Tier-3 query matches on `Environment + SourceSystem` alone (by design, mirroring the quirk above) - so two *different* rows (e.g. different PracticeId) sharing the same Environment+SourceSystem would both match and crash `SingleOrDefaultAsync` instead of resolving cleanly. Added 3 filtered unique indexes (`PracticeRegistryEntryConfiguration.cs`, migration `AddPerTierUniqueIndexesOnPractices`) that each mirror one tier's exact WHERE clause, so this ambiguity is rejected at insert/update time. Caught and fixed a first, too-narrow attempt live (a test insert of a second `Environment='local'` row with a different PracticeId was wrongly allowed) before landing the corrected filters.
- ~~ERMS `GetScannedList`/`GetDischargeSummaryReportList` (AWS branch): `"Column 'DataType' is read only."` crash~~ — **Fixed 2026-07-28**: `ErmsDataRepository.GetOtherDocsAsync`/`GetDocResultsAsync` read `[HSS].[uspGetOtherDocs_AWS]`/`[HSS].[uspGetDocResults_AWS]` results via `DataTable.Load(reader)`, which copies SQL Server's schema-reported `IsReadOnly` flag onto the DataTable column - the AWS-branch procedures' `DataType` (and `Content`/`DocumentId`) columns come back flagged read-only (computed/derived in that proc's result shape) even though the equivalent non-AWS procedures don't have this issue, so the existing AWS-enrichment write (`row["DataType"] = ...`) threw instead of enriching. Fixed by explicitly clearing `column.ReadOnly = false` immediately before each enrichment write. Verified live: `GetScannedList` (practice 901, real AWS-enabled data) now returns 40+ real document records with real `DataType` values (`application/pdf`, `text/html`) correctly enriched, not a crash.
