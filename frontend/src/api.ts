...
export async function karoAuthenticate(
  ...
  return json?.status === "success"
    ? { ok: true, token: json.token, practiceId: json.practiceId, raw: json }
    : { ok: false, raw: json, error: json?.message ?? "Unknown error" };
}
...
export async function ermsAuthenticate(username: string, password: string, patientId: string, encounterId: string): Promise<AuthResult> {
  ...
  return tokenMatch
    ? { ok: true, token: tokenMatch[1], practiceId: practiceMatch?.[1], raw: text }
    : { ok: false, raw: text, error: errorMatch?.[1] ?? "Unknown error" };
}
...
export async function colAuthenticate(username: string, password: string, patientId: string, encounterId: string): Promise<AuthResult> {
  ...
  return json?.Token
    ? { ok: true, token: json.Token, practiceId: json.PracticeId, raw: json }
    : { ok: false, raw: json, error: json?.error ?? "Unknown error" };
}
